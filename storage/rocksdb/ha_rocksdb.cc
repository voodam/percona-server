...
static my_bool rocksdb_track_and_verify_wals_in_manifest = FALSE;
...
static my_bool rocksdb_enable_remove_orphaned_dropped_cfs = TRUE;
...
void rdb_gen_canonical_tablename(const std::string &db,
                                 const std::string &table,
                                 const std::string &partition,
                                 std::string &result) {
  result = "./";
  ...
  if (partition.length() > 0)
    result.append(std::string(RDB_PARTITION_STR) + partition);
}
...
int ha_rocksdb::truncate_table(Rdb_tbl_def *tbl_def_arg, const TABLE *table_arg,
  ...
  */
  ...
  std::string tmp_tablename, canonical_tmp_tablename, canonical_tablename;
  rdb_gen_canonical_tablename(dbname, tblname, partition, canonical_tablename);
  tblname = std::string(TRUNCATE_TABLE_PREFIX) + tblname;
  rdb_gen_normalized_tablename(&dbname, &tblname, &partition, &tmp_tablename);
  rdb_gen_canonical_tablename(dbname, tblname, partition,
                              canonical_tmp_tablename);

  err = rename_table(canonical_tablename.c_str(),
                     canonical_tmp_tablename.c_str());
  if (err != HA_EXIT_SUCCESS) DBUG_RETURN(err);
  ...
  */
  ...
  if (err != HA_EXIT_SUCCESS) {
    int rename_err = rename_table(canonical_tmp_tablename.c_str(),
                                  canonical_tablename.c_str());
    ...
    */
    if (rename_err == HA_EXIT_SUCCESS) {
      ...
      sql_print_error(
          ...
          canonical_tablename.c_str(), canonical_tmp_tablename.c_str());
      err = rename_err;
    }
  }
...
#if 1  // Percona Server disabled rocksdb_read_free_rpl_tables as it's dangerous to use
  return true;
#else
